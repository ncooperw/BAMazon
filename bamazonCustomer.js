...
function start() {
    connection.query("SELECT * FROM Products", function (err, res) {
        ...
        inquirer.prompt([{
            ...
            var grandTotal = parseFloat(((res[whatToBuy].Price) * amountToBuy).toFixed(2));
            console.log("Total: " + grandTotal);

            //check stock
            if (res[whatToBuy].StockQuantity >= amountToBuy) {
                //after purchase, updates quantity in Products
                connection.query("UPDATE products SET ? WHERE ?", [{
                        StockQuantity: (res[whatToBuy].StockQuantity - amountToBuy)
                    },
                    {
                        ItemID: ans.id
                    }
